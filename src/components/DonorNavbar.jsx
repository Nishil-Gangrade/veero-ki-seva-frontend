...
`}>
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-3">
            {/* Logo */}
            <div className={`transition-all duration-500 text-3xl font-extrabold bg-gradient-to-r from-orange-500 via-white to-green-600 bg-clip-text text-transparent drop-shadow-[0_2px_2px_white]
            ...
            {!isMobile && (
              <div className={`transition-all duration-500 flex items-center ${isScrolled ? "" : "bg-white text-black rounded-full px-6 py-2 space-x-8 text-base shadow-md"}`}>
                {!isScrolled && navLinks.map(({ to, label }) => (
                  <Link key={to} to={to} className="font-medium hover:text-orange-600 transition">
                    {label}
                  </Link>
                ))}
              </div>
      ...
      {/* === CURVED NAVBAR === */}
      {(isScrolled || isMobile) && (
        <div className={`fixed ${isMobile ? "bottom-0" : "top-0"} left-0 w-full z-40 transition-all duration-500`}>
          <div className="mx-auto max-w-sm md:max-w-md bg-gradient-to-r from-orange-500 via-white to-green-600 p-[2px] rounded-t-3xl md:rounded-b-3xl shadow-xl">
            <div className="flex justify-between items-center bg-black text-white rounded-t-3xl md:rounded-b-3xl px-6 py-3 text-sm">
